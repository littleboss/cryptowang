...
- E2 OMS-v2（`strategies/e2_demo_oms_v2.py`，Phase A 的同级模块）已入库：把 E2-G1 的闭环代码路径补齐——**仅 OKX 模拟盘** 的现货 **限价** 下单 / 撤单 / 改单（signed REST，标准库 `urllib`，只在 `DemoHttpSender` 一处），且**只在** `will_send_http=True` **并且** demo 检查全过时才发：`OKX_SIMULATED=1`（或 `OKX_FLAG=1`）+ 三个 `OKX_*` + base_url 在 demo 允许名单 + 每个请求强制 `x-simulated-trading: 1`。默认 `will_send_http=False`，行为与 Phase A 逐字节一致（同一份 plan、`sent=false`、`place/amend/cancel` 抛同名 `SendRefused`）。撤单 / 改单**只认** `clOrdId` 前缀 `e2g1v2` 的自家订单，demo 账户里那 8 笔连通性试单（模块外下的）默认永不被撤；不碰任何 `tradingBot` 端点（A / B′ / C 不动）；market / IOC / FOK、卖单（G1b 卖梯只做意向）、提现 / 划转全部在开 socket 前拒绝。**合 PR ≠ 上膛**：CI 只跑 dry-run 与拒绝路径，真实 demo 发送每次都要 `04-risk` + 用户明确确认（见 §14）。
- 本仓只入库纸面工具；**push ≠ 实盘**。
...
**本 PR 明确不含且被代码拒绝**：任何写端点（下单 / amend / cancel）、提现 / 划转、市价 / IOC 扫单、自动重画区间、默认 `will_send_http=true`、T1 / T2 套利 live 路径、对官方 Bot 的任何操作。**Phase A 绿 ≠ 允许 demo 下单**；demo 执行器见 §14（另一模块、另一道门）。

### 14. E2 OMS-v2 · 门禁化 OKX 模拟盘执行器（`strategies/e2_demo_oms_v2.py`）

**OMS-v2 = Phase A 意向构建器 + 一条只通往 OKX 模拟盘的发送路径。** 对应 `04-risk` E2-OMS-v2：允许提代码 PR；**merge ≠ auto-fire**；A / B′ / C 官方 Bot 不动；不做 T1 / T2；不做市价 / IOC 扫单；不做提现 / 划转。这是仓内**唯一**含交易 HTTP 的模块（标准库 `urllib`，只在 `DemoHttpSender._request` 一处调用 `urlopen`；单测 + CI 用 AST 断言）。Phase A 模块 `e2_demo_oms.py` 保持零 HTTP import，plan 构建全部复用它。

**闭环代码路径**（每一步都可被代码拒绝，任何一步拒绝 → 不开 socket）：

```
GridIntentConfig ──► DemoOmsV2.build_plan ──► phase_a.build_intent_plan（同一份 plan，plan_id 相同）
                          │  (armed 时先跑 arm_check：flag → env → host，再做 Phase A 预检)
                          ▼
                  DemoOmsV2.submit(plan)
                          │  will_send_http=False ──► ExecutionReport(sent=false)   ← 默认，与 Phase A 一致
                          │  will_send_http=True  ──► verify_plan（schema / plan_id 重算 / ≤10 档 / ≤100 USDT）
                          │                          → 每档 order body 预检（ETH-USDT / cash / buy / limit|post_only /
                          │                            px < ref / sz ≥ minSz / clOrdId 前缀 e2g1v2 / tag e2g1omsv2 / 无多余字段）
                          │                          → 实例配额（≤10 单、≤100 USDT、同 plan 不重发）
                          ▼
                  DemoHttpSender._request ──► assert_endpoint_allowed（5 个端点白名单 + 片段黑名单）
                          │                  → 签名头（HMAC 复用 okx_readonly_client.sign）+ 强制 x-simulated-trading: 1
                          │                  → 开 socket 前再查一次该头（SimulatedHeaderMissing）
                          ▼
                  POST /api/v5/trade/order   （cancel-order / amend-order 同路；GET orders-pending 只用于 cancel_tagged）
```

| 门禁 | 实现 |
|------|------|
| 默认不发 | `DemoOmsV2(will_send_http=False)`、`POLICY.will_send_http=false`、`POLICY.trading_http` 全 false、`merge_is_arming=false`。禁用态下：`build_plan` 输出与 `e2_demo_oms.build_intent_plan` **逐字节相等**；`submit` → `sent=false`；`place_order / amend_order / cancel_order / cancel_tagged` 抛 Phase A 同名 `SendRefused`（`PlaceOrderRefused` / `AmendRefused` / `CancelRefused`）；`GridIntentConfig(will_send_http=True)` 或篡改 plan 的 flag **不能**上膛（`SendNotArmed`）——实例参数是唯一开关 |
| `will_send_http=True` 三道 demo 检查 | ① env：三个 `OKX_*` + `OKX_SIMULATED=1`（别名 `OKX_FLAG=1`），否则 `DemoEnvCheckFailed`，输出只报 set/unset；② host：`base_url` ∈ `{https://www.okx.com}`（OKX demo 与 live 同 host，靠 header 区分）或本机 `http://127.0.0.1 / localhost`（仅供单测），其余（`aws.okx.com` / `eea.okx.com` / 带路径 / `http://www.okx.com` / 任何别的域）→ `LiveHostRefused`；③ header：`x-simulated-trading: 1` 在 `_headers` 强制写入，`urlopen` 之前再校验一次，缺失 → `SimulatedHeaderMissing`。`live=True` 永远最先拒（`LiveFlagRefused`） |
| 端点白名单 | 只有 `POST trade/order` · `POST trade/cancel-order` · `POST trade/amend-order` · `GET trade/orders-pending` · `GET trade/order`；路径含 `batch` / `tradingbot` / `algo` / `withdraw` / `transfer` / `/asset/` / `sub-account` / `close-position` / `margin` / `position` / `set-` / `convert` / `rfq` / `block` / … → `EndpointForbidden`（先于 I/O） |
| 订单体预检 | `InstrumentNotApproved`（非 ETH-USDT）、`TdModeForbidden`（非 cash）、`SellSendNotArmed`（`side=sell`：卖梯只做意向）、`SideForbidden`、`OrderTypeForbidden`（market / ioc / fok / optimal_limit_ioc）、`BuyLevelAboveReference`（px ≥ ref）、`SizeBelowMin`、`NotionalOverCap`（单笔与实例累计都 ≤ 100）、`LevelsOverCap`（实例累计 ≤ 10）、`UnknownOrderField`（如 `attachAlgoOrds` / `reduceOnly`）、`OrderTagMismatch`、`DuplicateSubmit`（同 plan 二次提交） |
| **只动自家单** | 撤单 / 改单必须给 `clOrdId` 且以 `e2g1v2` 开头（`ForeignOrderRefused`），**不接受 `ordId`**；Phase A 意向 id（`e2g1a…`）、G1b 卖意向 id（`e2g1s…`）、任何模块外的单都算 foreign。`cancel_tagged` = `GET orders-pending` → 前缀过滤 → 逐笔 `POST cancel-order`，报告 `own_seen / foreign_skipped / cancelled`；demo 账户里那 **8 笔连通性试单默认永不被撤**（单测用 8 笔 foreign + 自家单的假 pending 列表断言）。任何 `tradingBot` grid stop / amend 端点不存在于本模块 |
| 秘密不回显 | `DemoCredentials.__repr__` 打码；`audit` 只记 method / path / clOrdId；`DemoApiError` 只带 OKX `code / sCode / msg`；CLI 拒绝 / 错误 JSON 里没有任何 env 值（CI 用占位 key 断言 `grep` 不到） |
| 错误处理 | OKX 返回 `code≠0` 或 `sCode≠0` → `DemoApiError`；`submit` 在**第一笔**失败处停止（`orders_placed` / `orders_failed` / `results[*].sCode` 如实报告），不重试、不改单补救 |
| G1b 卖梯（可选 stub） | `build_sell_intents(plan, filled_levels)`：对已成交买档在 `paired_exit_px`（高一档）生成同量 **卖出限价意向**，`clOrdId` 前缀 `e2g1s`、`sendable=false` / `will_send_http=false`；**不会自动发**，armed 的 OMS 也拒绝 `side=sell`（`SellSendNotArmed`）。要发卖单须另开 `04-risk` |

```bash
# 默认 = dry-run：与 Phase A 同一份 plan，execution.sent=false，什么都不发（exit 0）
uv run python strategies/e2_demo_oms_v2.py plan --out /tmp/e2-v2-run.json
# --will-send-http 仍要过 demo 检查：缺 env → DemoEnvCheckFailed；非 demo host → LiveHostRefused（都是 exit 3，不回显）
uv run python strategies/e2_demo_oms_v2.py plan --will-send-http
OKX_SIMULATED=1 OKX_API_KEY=… OKX_API_SECRET=… OKX_API_PASSPHRASE=… \
  uv run python strategies/e2_demo_oms_v2.py plan --will-send-http --base-url https://aws.okx.com
# 三者齐全（且已获 04-risk + 用户确认）才会向 OKX 模拟盘下 ≤10 笔限价买单；输出 execution.results[*].clOrdId（e2g1v2…）
OKX_SIMULATED=1 OKX_API_KEY=… OKX_API_SECRET=… OKX_API_PASSPHRASE=… \
  uv run python strategies/e2_demo_oms_v2.py plan --will-send-http --levels 3 --total-notional 30 --out /tmp/e2-v2-run.json
# 改单 / 撤单 / 撤全部自家单：不带 --will-send-http 只打印 body 预览（exit 0，http_sent=false）；foreign id → exit 3
uv run python strategies/e2_demo_oms_v2.py amend --cl-ord-id e2g1v2… --new-px 2305 --ref-px 2500
uv run python strategies/e2_demo_oms_v2.py cancel --cl-ord-id e2g1v2…
uv run python strategies/e2_demo_oms_v2.py cancel-tagged            # 带 --will-send-http 才会 GET pending 并撤自家单；8 笔试单 skipped
# G1b 卖梯意向（离线，永不发送）
uv run python strategies/e2_demo_oms_v2.py sell-intents --plan-json /tmp/e2-v2-run.json --filled-levels 1,2
uv run python strategies/e2_demo_oms_v2.py policy
uv run python strategies/e2_demo_oms_v2.py check-env               # 只报 set/unset
```

产出 `e2_demo_oms_v2_run_v1`：顶层 `mode`（`dry_run` / `demo_send`）、`will_send_http`、`http_sent`、`orders_placed`、`policy`、`plan`（原样的 `e2_demo_oms_intent_plan_v1`）、`execution`（`e2_demo_oms_v2_execution_report_v1`：`sent / http_sent / orders_placed / orders_failed / reason / requests_made / base_url / results[]`）、`risk_notes`。退出码：0 正常（含 dry-run）、3 拒绝、1 OKX / 网络错误（`http_sent` 如实标注）。

**执行仍需风控 + 用户确认。** 本 PR 交付的是**代码路径**与它的拒绝面，不是放行：`ARMING_REQUIRES` 写在 `POLICY` 里——`04-risk` 针对具体 demo 运行的放行、用户在聊天里的明确确认、demo env、命令行上的 `--will-send-http`、demo host，五项缺一不发。CI 从不上膛（占位 key 只用来证明拒绝路径不回显；默认 demo host 在 CI 里**不**配 `--will-send-http` 跑）。**明确不含且被代码拒绝**：live、market / IOC / FOK、提现 / 划转、任何 `tradingBot` / algo 端点（官方 Bot A / B′ / C 不动）、T1 / T2 执行、自动重画区间、自动发卖单、`ordId` 操作、非 `e2g1v2` 前缀的撤改。
...
2. 十四个脚本的 `--help` 能通过 `uv run --locked --no-dev python …` 启动（Python 3.12）。
...
13. `e2_demo_oms_v2.py`（E2 OMS-v2 demo 执行器，**CI 从不上膛**）：清空 `OKX_*` 后默认 `plan` 断言 `schema=e2_demo_oms_v2_run_v1`、`mode=dry_run`、`will_send_http=false` / `http_sent=false` / `orders_placed=0` / `execution.sent=false`、policy `will_send_http=false` / `merge_is_arming=false` / `trading_http` 全 false / armed 态 `withdraw` `transfer` 仍 false / `allowed_base_urls=[https://www.okx.com]` / 强制头 / 只可发 `buy` / 1x / Bot 不动、内嵌 plan 与 **Phase A 同次运行的 plan 逐字节相等**、文本无禁词；`--will-send-http` 无 env → 退出 3 `DemoEnvCheckFailed`；占位（非密钥）demo env + `--will-send-http` 下 `--base-url https://aws.okx.com` / `https://evil.example` → `LiveHostRefused`、`--live` → `LiveFlagRefused`、`--total-notional 500` → `NotionalOverCap`、`cancel` / `amend` 给 foreign 或 Phase A 前缀 `clOrdId` → `ForeignOrderRefused`，全部退出 3、`http_sent=false` 且 `grep` 不到占位 key（默认 demo host **不**带 `--will-send-http` 跑）；不带 flag 的 `cancel` / `amend` / `cancel-tagged` 退出 0 且 `http_sent=false`；AST 扫描 OMS-v2 源码：允许 `urllib`，禁 `http` / `socket` / `httpx` / `requests` / `okx` / `websockets`，`urlopen` **只有一个调用点**且位于 `DemoHttpSender._request`，源码无 `algoId` / `/tradingBot/` / `okx.Trade` / `batch-orders`；`unittest.mock` 同时挂在 `urllib.request.urlopen` 与 `DemoHttpSender._request` 上，断言禁用态 build → submit → place/amend/cancel 拒绝、armed + 无 env、armed + 非 demo host 全程**从未开 socket**。
14. `python -m unittest discover -s tests`（无网络；OMS-v2 的 armed 路径只对本机 `http.server` 假 demo 服务器跑：校验 HMAC 签名与 `x-simulated-trading: 1`，8 笔 foreign 挂单永不被撤）。
15. 各跑一遍默认参数：`okx_grid_dry_run.py` 必须含 `"will_send_http": false` 和 `method: PRINT_ONLY`。
